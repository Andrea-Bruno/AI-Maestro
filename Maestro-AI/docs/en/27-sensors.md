...
All sensor and heating tools are available in the **Strumenti (Tools)** tab:

- **NIR Spectrometer**: Enter session ID, wavelengths (comma-separated), and intensities (comma-separated) → click *Record* to store a spectral sample. Click *Get Recent* to retrieve the last 5 spectra.
- **Crack Detection**: Feed amplitude readings and time values. Use *Set Threshold* to adjust sensitivity, *Reset* to clear the crack counter before a new roast.
- **Hybrid Heating**: Set the power distribution between traditional, microwave, and infrared sources (must sum to 100). Click *Apply* to send the configuration, *Status* to read the current heating mode.
